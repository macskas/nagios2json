...
static void format_date(uint32_t *diff, std::string *buffer)
{	
	static char tbuffer[512];
	static int day, hour, min;
	day = hour = min = 0;
...
static void format_status_service(uint8_t *st, std::string *buffer)
{
	if (*st & SERVICE_OK) {
		buffer->assign("OK");
...
NagiosParser::NagiosParser(std::string filepath)
{
...
void NagiosParser::parse()
{
	...
	char				*buffer_key = NULL, *buffer_value = NULL;
	char				*eq_helper = NULL;
	...
	char *buffer_newline = NULL;
	
	while (1)
	{
		buffer_newline = strchr(buffer_current, '\n');
		if (buffer_newline == NULL)
			break;
		*buffer_newline = 0;
			...
			switch (category)
			{
				...
				case C_SERVICESTATUS:
					if (key_length == 11 && strncmp(buffer_key, "is_flapping", 11) == 0) {
						...
						servicedata.last_state_change = strtoull(buffer_value, NULL, 10);
						servicedata.last_state_diff = now - servicedata.last_state_change;
					} else if (key_length == 12 && strncmp(buffer_key, "max_attempts", 12) == 0) {
						servicedata.max_attempts = strtoull(buffer_value, NULL, 10);
					} else if (key_length == 15 && strncmp(buffer_key, "current_attempt", 15) == 0) {
						servicedata.current_attempt = strtoull(buffer_value, NULL, 10);
				...
				case C_INFO:
					if (key_length == 7 && strncmp(buffer_key, "created", 7) == 0) {
						now = strtoull(buffer_value, NULL, 10);
						this->created = now;
					}
				break;
				...
				break;
					if (key_length == 9 && strncmp(buffer_key, "host_name", 9) == 0) {
						...
						nagios_downtime.added = strtoull(buffer_value, NULL, 10);
					} else if (key_length == 10 && strncmp(buffer_key, "start_time", 10) == 0) {
						nagios_downtime.start_time = strtoull(buffer_value, NULL, 10);
					} else if (key_length == 9 && strncmp(buffer_key, "end_time", 8) == 0) {
						nagios_downtime.end_time = strtoull(buffer_value, NULL, 10);
						...
						nagios_downtime.duration = strtoull(buffer_value, NULL, 10);
				...
				case C_HOSTCOMMENT:
					if (key_length == 19 && strncmp(buffer_key, "service_description", 19) == 0) {
						...
						nagios_comment.added = strtoull(buffer_value, NULL, 10);
					} else if (key_length == 7 && strncmp(buffer_key, "comment", 7) == 0) {
...
void NagiosParser::add_servicecomment(std::string hostname, std::string servicename, nagios_comment_t *d)
{
...
void NagiosParser::add_servicedowntime(std::string hostname, std::string servicename, nagios_downtime_t *d)
{
	static std::string concat;
	static nagiosmap_service_t::iterator it_c;
	concat.assign(hostname);
...
void NagiosParser::add_hostcomment(std::string hostname, nagios_comment_t *d)
{
...
void NagiosParser::add_hostdowntime(std::string hostname, nagios_downtime_t *d)
{
	static nagiosmap_host_t::iterator it_c;
	it_c = this->hosts.find(hostname);
	if (it_c == this->hosts.end())
...
void NagiosParser::do_json()
{
	...
	for (nagiosmap_service_t::iterator it_c=this->services.begin(), it_e=this->services.end(); it_c != it_e; ++it_c)
	{
		...
		snprintf(buffer, 4096,
			...
			((scurr->hostdata != NULL && (scurr->hostdata->status & HOST_UP) != 0)  ? 1 : 0),
			(scurr->hostdata != NULL ? scurr->hostdata->services : 0),
			(scurr->hostdata != NULL ? scurr->hostdata->services_visible : 0),
			diffbuffer.c_str()
		);
...
void NagiosParser::filter()
{
	...
	while (it_current != it_end)
	{
		...
		if (hcurr != NULL && hostprops != 0 && ((hcurr->flags & hostprops) != hostprops)) {
				show = false;
		}
		if (show) {
			++it_current;
