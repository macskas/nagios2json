...
static bool is_numeric(char *d)
{
	for (int i=0,e=strlen(d); i<e; ++i)
	{
		if (!isdigit(d[i]))
			return false;
	}
...
static void find_statusfile()
{
	...
	while (*cl != NULL)
	{
		...
		while (filestr.getline(buffer, buffer_max))
		{
			...
			if (cur_helper == NULL || cur_helper-buffer == 0)
				continue;
			cur_helper++;
...
int main(int argc, char **argv)
{
	...
	query_string = getenv("QUERY_STRING") ? strdup(getenv("QUERY_STRING")) : NULL;
